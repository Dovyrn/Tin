...
pub struct Surface {
    ...
    source: Option<Retained<ProtocolObject<dyn MTLTexture>>>,
    width: u32,
    height: u32,
}
...
impl Surface {
    pub fn new(device: &Device, window: &NSWindow) -> Self {
        ...
        Self { layer, drawable: None, source: None, width: 0, height: 0 }
    }
    ...
    pub fn blit(&mut self, source: &ProtocolObject<dyn MTLTexture>) {
    ...
    pub fn present(&mut self, device: &Device) {
        ...
        match self.source.take() {
            Some(source) => {
                ...
                unsafe {
                    blit.copyFromTexture_toTexture(&source, &drawable.texture());
                }
                blit.endEncoding();
            }
            None => {
